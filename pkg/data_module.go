...
func recordFieldsImp(thread *Thread, registers []Val, stack *Stack, pos Pos) error {
...
func recordLengthImp(thread *Thread, registers []Val, stack *Stack, pos Pos) error {
	stack.Push(NewVal(&TInt, Int(stack.Pop().data.(*Record).Len())))
	return nil
}

func recordMergeImp(thread *Thread, registers []Val, stack *Stack, pos Pos) error {
	source := stack.Pop().data.(*Record)
	stack.Pop().data.(*Record).Merge(source)
	return nil
}

func recordSetImp(thread *Thread, registers []Val, stack *Stack, pos Pos) error {
